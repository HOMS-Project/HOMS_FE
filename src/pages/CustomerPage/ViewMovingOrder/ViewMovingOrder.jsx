    ...
    import {
        Layout, Table, Tag, Empty, Button, Tabs, Row, Col, Typography, Image, Modal, message, Divider, Tooltip,Form,Select,Input,Upload, Progress
    } from "antd";
...
const handleViewIncident = (incident) => {
    ...
    // Giả lập "done" ngay để Ant Design hiển thị preview, 
    // file thật sẽ được gửi khi user bấm "Gửi báo cáo"
  ...
  >
    ...
    confirmLoading={isSubmittingReport} // Hiển thị loading ở nút OK
    okText="Gửi báo cáo"
    cancelText="Hủy"
   onOk={() => {
    reportForm.validateFields().then(async (values) => {
        try {
            ...
            if (selectedTicket?.invoice?._id) {
    formData.append("invoiceId", selectedTicket.invoice._id);
}
...
>
    <Form form={reportForm} layout="vertical">
        ...
        <Form.Item label="Hình ảnh/Video (nếu có - tối đa 5 file)">
           <Upload
    ...
    Modal.info({
      ...
      content: isVideo ? (
        <video controls style={{ width: "100%" }}>
          <source src={URL.createObjectURL(file.originFileObj)} />
        </video>
      ) : (
...
>
  {selectedIncident && (
    <>
      <p><b>Loại:</b> {selectedIncident.type}</p>
      <p><b>Mô tả:</b> {selectedIncident.description}</p>

      <p>
        <b>Trạng thái:</b>{" "}
        <Tag color="orange">{selectedIncident.status}</Tag>
      </p>

     {selectedIncident.images?.map((file, i) => {
  const isVideo = file.match(/\.(mp4|webm|ogg|mov)$/i);

  return isVideo ? (
    <video key={i} width={200} controls>
      <source src={file} />
    </video>
  ) : (
    <Image key={i} src={file} width={120} />
  );
})}
      ...
      {selectedIncident.status === "Resolved" && (
        <>
          ...
          <p>
            <b>Kết quả xử lý:</b>{" "}
            {selectedIncident.resolution?.action}
          </p>

          {selectedIncident.resolution?.compensationAmount > 0 && (
            <p>
              Bồi thường:{" "}
              {selectedIncident.resolution.compensationAmount.toLocaleString()} ₫
            </p>
          )}
        </>
      )}
